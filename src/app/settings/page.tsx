...
export default function Settings() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user?.email) {
        try {
          const res = await fetch(`/api/getPostColor?email=${encodeURIComponent(user.email)}`);
    ...
    <>
      ...
      <div className="min-h-screen px-5 md:px-20 py-10 bg-gradient-to-br from-gray-50 via-white to-gray-100">
        <div className="max-w-3xl mx-auto">
          ...
          {notification && (
            <div className={`mb-6 p-4 rounded-lg border-l-4 shadow-sm transition-all duration-300 ${
          ...
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-200 hover:shadow-xl transition-shadow duration-300">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-2xl font-semibold text-gray-800">Default Post Color</h3>
                <p className="text-sm text-gray-500">Set a color that will be applied to all your posts</p>
              </div>
            </div>
              ...
              <label className="flex items-center justify-between cursor-pointer group">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {useDefaultColor ? 'Your posts will use the selected color' : 'Posts will use their default colors'}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xl font-bold text-gray-800">{defaultColor.toUpperCase()}</span>
                    </div>
                    <div className="h-8 rounded-lg shadow-inner" style={{backgroundColor: defaultColor}}></div>
                  </div>
                </div>
              </div>
            )}
