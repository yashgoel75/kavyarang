...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const normalizedImages = pool.map((image) => {
    if (typeof image === "string") {
      return { src: image, alt: "", username: undefined, name: undefined };
    }
    return {
      src: image.src || "",
      alt: image.alt || "",
  ...
  return coords.map((c, i) => ({
    ...
    username: usedImages[i].username,
...
export default function DomeGallery({
  ...
  const items = useMemo(
    () => buildItems(userImages, segments),
    [userImages, segments]
  );
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const mapped: ImageItem[] = data.users
          .filter((u: User) => u.profilePicture)
          .map((u: User) => ({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (parent.dataset.username) {
      const infoDiv = document.createElement("div");
      infoDiv.className = "user-info";
      infoDiv.style.cssText = `position:absolute; bottom:0; left:0; right:0; background:linear-gradient(to top, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.7) 50%, transparent 100%); color:white; padding:24px 20px; cursor:pointer; z-index: 1000; pointer-events: auto;`;
      infoDiv.innerHTML = `<div style="font-size:18px; font-weight:600;">${
        parent.dataset.name || ""
      }</div><div style="font-size:14px; opacity:0.9;">@${
        parent.dataset.username
      }</div>`;
      infoDiv.onclick = (e) => {
        e.stopPropagation();
        window.open(`/user/${parent.dataset.username}`, "_blank");
      };
